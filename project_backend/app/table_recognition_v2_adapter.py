...
def _predict_table_model(model: Any, image: np.ndarray) -> Any:
    ...
    try:
        ...
        if "raw_output" in remote_result:
            return remote_result["raw_output"]
        return remote_result.get("output", remote_result)
...
def recognize_table_v2_local(image: np.ndarray) -> Dict[str, Any]:
    ...
    logger.info("Using remote Table model runtime with backend table process logic")
