...
AUTO_ROI_TABLE_EXPAND_RIGHT_PX = float(os.getenv("AUTO_ROI_TABLE_EXPAND_RIGHT_PX", "6"))
AUTO_ROI_MAX_NEIGHBOR_OVERLAP_RATIO = float(os.getenv("AUTO_ROI_MAX_NEIGHBOR_OVERLAP_RATIO", "0.1"))
AUTO_ROI_TABLE_NESTED_OVERLAP_RATIO = float(os.getenv("AUTO_ROI_TABLE_NESTED_OVERLAP_RATIO", "0.9"))
AUTO_ROI_TABLE_NESTED_AREA_RATIO = float(os.getenv("AUTO_ROI_TABLE_NESTED_AREA_RATIO", "1.0"))
AUTO_ROI_IMAGE_NESTED_OVERLAP_RATIO = float(os.getenv("AUTO_ROI_IMAGE_NESTED_OVERLAP_RATIO", "0.9"))
AUTO_ROI_IMAGE_NESTED_AREA_RATIO = float(os.getenv("AUTO_ROI_IMAGE_NESTED_AREA_RATIO", "1.02"))
...
def _get_cached_layout_raw_items(image: np.ndarray, source_mode: str) -> Optional[List[Dict[str, Any]]]:
    ...
    logger.info("Layout diagnostic: raw_items_cache_hit shape=%s item_count=%s", image.shape, len(cached))
...
def _set_cached_layout_raw_items(image: np.ndarray, raw_items: List[Dict[str, Any]], source_mode: str) -> None:
    ...
    while len(_layout_raw_items_cache) > LAYOUT_RAW_ITEMS_CACHE_SIZE:
        _layout_raw_items_cache.popitem(last=False)


def clear_layout_analysis_cache() -> None:
    cleared_count = len(_layout_raw_items_cache)
    _layout_raw_items_cache.clear()
    if cleared_count:
        logger.info("Layout diagnostic: raw_items_cache_cleared count=%s", cleared_count)
...
def _filter_nested_same_type_regions(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in ordered:
        ...
        for existing in kept:
            ...
            nested_image_shell = (
                region_type == "image"
                and overlap_ratio >= AUTO_ROI_IMAGE_NESTED_OVERLAP_RATIO
                and area_ratio <= AUTO_ROI_IMAGE_NESTED_AREA_RATIO
            )
            if nested_same_type or nested_table_shell or nested_image_shell:
                nested = True
                logger.debug(
