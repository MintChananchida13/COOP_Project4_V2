...
def _extract_rows(result: Dict[str, Any]) -> List[List[str]]:
    ...
    if isinstance(structured, dict) and isinstance(structured.get("cells"), list):
        ...
        if rows:
            return rows
    for key in ("rows", "table_rows", "cells", "table_cells", "cell_bbox", "cell_bboxes"):
...
def _extract_structured_table(result: Dict[str, Any], rows: List[List[str]], html: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(structured, dict):
        return structured
    for key in ("cells", "table_cells", "cell_bbox", "cell_bboxes"):
...
def _infer_cell_bboxes_from_grid(rows: List[List[str]], image: np.ndarray) -> List[Dict[str, Any]]:
    if not rows:
        return []
    height, width = image.shape[:2]
    row_count = len(rows)
    ...
    for row_index in range(row_count):
        for col_index in range(col_count):
            left = int(round(width * col_index / col_count))
            right = int(round(width * (col_index + 1) / col_count))
            top = int(round(height * row_index / row_count))
            bottom = int(round(height * (row_index + 1) / row_count))
            cells.append(
                {
                    "row": row_index,
                    "col": col_index,
                    ...
                    "rowSpan": 1,
                    "colSpan": 1,
                    "bbox": {"x": left, "y": top, "width": max(1, right - left), "height": max(1, bottom - top)},
                    "bboxSource": "backend_grid_inference",
                }
            )
    return cells


def _apply_backend_ocr_core_to_remote_cells(result: Dict[str, Any], image: np.ndarray) -> Dict[str, Any]:
    if not _use_remote_runtime():
        return result
    raw_results = result.get("raw_results")
    if not (
        isinstance(raw_results, list)
        and any(isinstance(item, dict) and isinstance(item.get("table_structured"), dict) for item in raw_results)
    ):
        return result
    structured = result.get("table_structured")
    if not isinstance(structured, dict):
        return result
    rows = normalize_table_rows(result.get("table_rows") or structured.get("rows") or [])
    cells = _normalize_cell_dicts(structured.get("cells"))
    mapping_method = "slanext_cell_bbox_to_backend_ocr_core"
    bbox_inference: Dict[str, Any] = {"enabled": False, "inferred_bbox_count": 0}
    if cells and not any(isinstance(cell.get("bbox"), dict) for cell in cells):
        cells = _infer_cell_bboxes_from_grid(rows, image)
        bbox_inference = {"enabled": True, "method": "uniform_grid_from_slanext_rows", "inferred_bbox_count": len(cells)}
        mapping_method = "backend_grid_inference_to_backend_ocr_core"
    ocr_cells = [cell for cell in cells if isinstance(cell.get("bbox"), dict) and not cell.get("hidden")]
    if not ocr_cells:
        return result

    height, width = image.shape[:2]
    crops: List[np.ndarray] = []
    crop_cells: List[Dict[str, Any]] = []
    for cell in ocr_cells:
        bbox = cell.get("bbox")
        if not isinstance(bbox, dict):
            continue
        ...
        right = min(width, x + max(1, int(round(float(bbox.get("width") or 1)))))
        bottom = min(height, y + max(1, int(round(float(bbox.get("height") or 1)))))
    ...
    recognitions, ocr_debug = _recognize_text_crops_with_core(crops, "remote_table_cell_text")
    next_cells = [dict(cell) for cell in cells]
    by_position = {(int(cell.get("row") or 0), int(cell.get("col") or 0)): cell for cell in next_cells}
    confidence_values: List[float] = []
    for cell, recognition in zip(crop_cells, recognitions):
        text = normalize_ocr_text(recognition.get("text") if isinstance(recognition, dict) else "")
        confidence = float(recognition.get("confidence") or 0.0) if isinstance(recognition, dict) else 0.0
        if confidence:
            confidence_values.append(confidence)
        target = by_position.get((int(cell.get("row") or 0), int(cell.get("col") or 0)))
        if target is None:
            continue
        target["text"] = text
        target["ocrText"] = text
        target["groundTruth"] = text
        target["confidence"] = confidence
        target["assignmentSource"] = "backend_ocr_core"
        if cell.get("bboxSource"):
            target["bboxSource"] = cell.get("bboxSource")

    next_rows = _rows_from_structured_cells_preserve_grid(next_cells)
    if next_rows:
        result["table_rows"] = next_rows
        result["text"] = _markdown_table(next_rows)
    result["table_structured"] = {**structured, "rows": next_rows or rows, "cells": next_cells}
    if confidence_values:
        result["confidence"] = sum(confidence_values) / len(confidence_values)
    debug = result.get("table_debug")
    if not isinstance(debug, dict):
        debug = {}
    debug["backend_ocr_core"] = {
        **(ocr_debug if isinstance(ocr_debug, dict) else {}),
        "mapping_method": mapping_method,
        "cell_count": len(crop_cells),
        "bbox_inference": bbox_inference,
    }
    result["table_debug"] = debug
    return result
...
def _recover_slanext_structure_collapse(candidate: Dict[str, Any], image: np.ndarray) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    ocr_geometry_fallback: Dict[str, Any] = {
        ...
        "missing_column_boundaries": missing_col_boundaries,
        "y_cluster_count": 0,
        "x_cluster_count": 0,
    }
    if missing_row_boundaries or missing_col_boundaries:
        ...
        ocr_geometry_fallback["y_cluster_count"] = len(all_y_clusters)
        ocr_geometry_fallback["x_cluster_count"] = len(all_x_clusters)
        if len(all_y_clusters) <= body_row_count:
            return candidate, {
                **base_debug,
                "attempted": True,
                "body_row_count": body_row_count,
                ...
                "recovered_row_count": row_count,
                "recovered_column_count": col_count,
                "reason": "missing_boundaries_without_ocr_row_evidence",
                "ocr": ocr_debug,
        ...
        if len(col_boundaries) < col_count + 1:
            col_boundaries = [0.0, *[value for value in col_boundaries[1:-1]], float(image.shape[1])]

    body_top = row_boundaries[header_row_count] if header_row_count < len(row_boundaries) else row_boundaries[0]
    ...
    if (missing_row_boundaries or missing_col_boundaries) and len(_cluster_ocr_rows_by_y(body_ocr_cells)) <= body_row_count:
        all_clusters = _cluster_ocr_rows_by_y(ocr_cells)
        body_clusters = all_clusters[header_row_count:] if len(all_clusters) > header_row_count else all_clusters
        body_ocr_cells = [cell for cluster in body_clusters for cell in cluster]
